...
private function ApplyGravityAndJumping (velocity : Vector3) {
	...
	if (grounded) {
		...
		if (jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2)) {
			...
			if (jumping.enabled && canControl && (Time.time - jumping.lastButtonDownTime < 0.2) && (jumping.holdingJumpButton == false)) {
			...
			// Calculate the jumping direction
			velocity.y = 0;
			var timefactor : float = 1f * (Mathf.Sin(Time.time*8f));
